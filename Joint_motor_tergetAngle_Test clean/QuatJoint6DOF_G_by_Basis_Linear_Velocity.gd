...
func apply_rot_spring_quat(delta):# apply spring rotation using quaternion
	...
	var max_torque = 500000000
	#var max_torque = 0
	#var max_torque = 50000000
	
	var tb_consolidated = (redInertia)*(targetAng*stiffnessB/delta - (bAV-aAV)*dampingB)/delta
	#tb_consolidated = max_vector_magnitude(tb_consolidated,500000000)
	#var Jtb_consolidated = max_vector_magnitude(tb_consolidated,max_torque)*delta
	var Jtb_consolidated = (redInertia)*(targetAng*stiffnessB/delta - (bAV-aAV)*dampingB)
	...
	var trq = redInertia*(targetAng*stiffnessB/delta - (bAV-aAV)*dampingB)/delta
	trq = max_vector_magnitude(trq,50000)
	var tb_consolidated_v = redInertia.inverse()*trq*delta
	
	#var ang_v_b = targetAng*2
	var ang_v_b = targetAng*50
	#var torq_b = (redInertia)*(ang_v_b/delta)
	#var torq_b = clamp_vector((Ib_inv.inverse())*(ang_v_b/delta),-5000000,5000000)
	var torq_b = (Ib_inv.inverse())*(ang_v_b/delta)
	if(torq_b.length()>50000000):
		pass
	torq_b = max_vector_magnitude(torq_b,500000000)
	#torq_b = max_vector_magnitude(torq_b,5000000)
	#torq_b = max_vector_magnitude(torq_b,max_torque)
	ang_v_b = Ib_inv*torq_b*delta
	...
	if(get_parent().name == "LegG_REX_L"):
		...
		$RichTextLabel2.text= "tb_consolidated_v.length(): "+String(tb_consolidated_v.length())
#		$RichTextLabel3.text= "tb_consolidated: "+String(tb_consolidated)
	...
	if body_b.is_class("RigidBody") and body_b != null:
		...
		#body_b.apply_torque_impulse(Jtb_consolidated)
		#body_b.angular_velocity = ang_v_b
#		if(tb_consolidated.length()<max_torque):
#			body_b.angular_velocity = ang_v_b
		#else:
			#body_b.apply_torque_impulse(Jtb_consolidated)
