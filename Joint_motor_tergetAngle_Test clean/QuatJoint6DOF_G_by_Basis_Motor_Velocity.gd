...
func _physics_process(delta):
	if Engine.editor_hint:
		return
	if(hasBase):
		apply_motor_velocity()
		#apply_rot_spring_quat(delta)


func apply_motor_velocity():
	var target_rotation : Basis = get_node(puppeteer_node).global_transform.basis.inverse() * body_b.global_transform.basis
