...
#	self.set_param_x(PARAM_ANGULAR_MOTOR_FORCE_LIMIT,5000000000)
#	self.set_param_y(PARAM_ANGULAR_MOTOR_FORCE_LIMIT,5000000000)
#	self.set_param_z(PARAM_ANGULAR_MOTOR_FORCE_LIMIT,5000000000)
	...
	self.set_flag_z(FLAG_ENABLE_ANGULAR_SPRING,true)
	self.set_flag_x(FLAG_ENABLE_ANGULAR_SPRING,true)
	self.set_flag_y(FLAG_ENABLE_ANGULAR_SPRING,true)
	self.set_param_z(PARAM_ANGULAR_SPRING_STIFFNESS,100000)
	self.set_param_x(PARAM_ANGULAR_SPRING_STIFFNESS,100000)
	self.set_param_y(PARAM_ANGULAR_SPRING_STIFFNESS,100000)
	self.set_param_z(PARAM_ANGULAR_SPRING_DAMPING,100000)
	self.set_param_x(PARAM_ANGULAR_SPRING_DAMPING,100000)
	self.set_param_y(PARAM_ANGULAR_SPRING_DAMPING,100000)
...
export var torque_lim =50000
var prev_angl_b = Vector3(0,0,0)
var gravity_angl_init = false
func apply_rot_spring_quat(delta):# apply spring rotation using quaternion
	...
	#add_Basis_by_Element(redInertia,body_b.body_b.get_inverse_inertia_tensor()).inverse()
	...
	var pptr_basis = get_node(puppeteer_node).global_transform.basis
	var pptr_target_r_g = pptr_basis.get_rotation_quat()
	#var angle_pptr_gd = qTargetO.get_euler()
	#var angle_pptr_gd = pptr_target_r_g.get_euler()
	var angle_pptr_gd = pptr_basis.get_euler()
	
	
	var angle_pptr = 2.0 * acos(qTargetO.w)
	var v_pptr = Vector3(qTargetO.x,qTargetO.y,qTargetO.z)
	var axis_den_pptr = sin(angle*0.5)
	var axis_pptr = v_pptr / axis_den_pptr
	var targetAng_pptr = axis_pptr*angle_pptr
	
	var j_basis = self.global_transform.basis
	#var pptr_target_r_l = j_basis.inverse()*pptr_target_r_g
	#var b_target_r = self.global_transform.xform_inv(get_node(puppeteer_node).global_transform.basis.get_euler())
	...
	self.set_param_x(PARAM_ANGULAR_SPRING_EQUILIBRIUM_POINT,angle_pptr_gd.x)
	self.set_param_y(PARAM_ANGULAR_SPRING_EQUILIBRIUM_POINT,angle_pptr_gd.y)
	self.set_param_z(PARAM_ANGULAR_SPRING_EQUILIBRIUM_POINT,angle_pptr_gd.z)
	#self.set_param_z(PARAM_ANGULAR_SPRING_EQUILIBRIUM_POINT,-angle_pptr_gd.z)
...
#	I_clamp = Vector3(1,1,1) - I_clamp
	...
	var trq_joint_b = redInertia*(targetAng*stiffnessB/delta - (bAV-aAV)*dampingB)/delta
	trq_joint_b = max_vector_magnitude(trq_joint_b,torque_lim)
	var j_trq_joint_b = trq_joint_b*delta
	
	body_b.J_trq = j_trq_joint_b
	body_b.body_a = body_a
	body_b.ppt = puppeteer_node
	
	var mass_center_b = body_b.global_transform.origin#export
	#var trq_radius_vec = (self.global_transform.origin-mass_center_b)
	var trq_radius_vec = (mass_center_b-self.global_transform.origin)
	var gravity_vec = Vector3(0,-1,0)#export
	var vg = gravity_vec*9.8*delta
	#var Fg = gravity_vec*body_b.weight
	var mb = body_b.mass
	var Fg = gravity_vec*body_b.mass*9.8
	var trq_g_b = trq_radius_vec.cross(Fg)
	var Ib_point_mass = mb * trq_radius_vec.length_squared()
	#var mag = trq_joint_b.length() - trq_g_b.length()
	#trq_joint_b = trq_joint_b.normalized() * mag
	#var w_g_b = Ib_inv*trq_g_b
	var w_acc_g_b = trq_g_b/Ib_point_mass
	var w_g_b = trq_g_b*delta/Ib_point_mass
	
	var w_joint_b = redInertia.inverse()*(trq_joint_b)*delta
	body_b.w = w_joint_b
	#var w_joint_b = Ib_inv*(trq_joint_b)*delta
	var actual_angular_velocity = Vector3(0,0,0)
	if(gravity_angl_init):
		actual_angular_velocity = body_b.global_transform.basis.get_euler() - prev_angl_b
	else:
		gravity_angl_init = true
	
	prev_angl_b = body_b.global_transform.basis.get_euler()


	if(get_parent().name == "6DOF_G_BASIS_DEMO_IF"):
		...
		#$RichTextLabel3.text= "trq_joint_b.length(): "+String(trq_joint_b.length())
		#$RichTextLabel.text= "trq_joint_b + trq_g_b: "+String(trq_joint_b+trq_g_b)
		#$RichTextLabel.text= "trq_g_b: "+String(trq_g_b)
		#$RichTextLabel3.text= "trq_joint_b: "+String(trq_joint_b)
		#$RichTextLabel2.text= "w_joint_b.length(): "+String(w_joint_b.length())
		#$RichTextLabel.text= "trq_g_b.length(): "+String(trq_g_b.length())
		#$RichTextLabel2.text= "trq_g_b: "+String(trq_g_b)
		#$RichTextLabel2.text= "w_joint_b+w_g_b: "+String(w_joint_b+w_g_b)
		
		#$RichTextLabel.text= "actual_angular_velocity: "+String(actual_angular_velocity)
		#$RichTextLabel2.text= "body_b.angular_velocity: "+String(body_b.get_linear_velocity())
		#$RichTextLabel.text = "body_b.angular_velocity - w_g_b: "+String(body_b.angular_velocity - w_g_b)
		#$RichTextLabel.text = "trq_b_nat-trq_g_b: "+String((Ib_inv.inverse()*body_b.angular_velocity/delta)-trq_g_b)
	...
	if body_b.is_class("RigidBody") and body_b != null:
		...
		#body_b.angular_velocity = w_joint_b + w_g_b
		#body_b.angular_velocity = w_joint_b
		#var ang_v_b = targetAng*10
		#var ang_v_b = (targetAng*10)
		#body_b.angular_velocity = ang_v_b
		#body_b.angular_velocity += w_g_b
		
		
		#body_b.add_torque(trq_g_b)
		#body_b.add_torque(trq_joint_b)
		#body_b.add_torque(Vector3(0,0,-1)*10)
		#body_b.add_central_force(Fg)

		#body_b.linear_velocity += vg
		
		#body_b.apply_torque_impulse(trq_joint_b*delta)

#		self.set_param_x(PARAM_ANGULAR_MOTOR_TARGET_VELOCITY,w_joint_b.x)	#Causes Jitters for some reason
#		self.set_param_y(PARAM_ANGULAR_MOTOR_TARGET_VELOCITY,w_joint_b.y)
#		self.set_param_z(PARAM_ANGULAR_MOTOR_TARGET_VELOCITY,w_joint_b.z)
	...
	if body_a.is_class("RigidBody") and body_a != null:
		#body_a.apply_torque_impulse(-trq_joint_b*delta)
